...
app.controller('prmLoginAlmaMashupAfterController', ['$http', '$scope', '$sce', function ($http, $scope, $sce) {
  ...
  if (location.hostname === 'search.library.northwestern.edu') {
    ...
    // LINK_TEMPLATE = 'dev.galter.northwestern.edu:3000/search/results/view_it_primo?url=';
  }
  if ($scope.mashScope) {
    // A very bad hack to hide Primo's own viewit iframe
    // We're modifying prm-alma-mashup contriller we found earlier
    // *ducks*
    //FIXME - enable after testing resolvers side-by-side
    $scope.mashScope.isMashupLink = function () { return false }
 ...
 * Enable for append-builder so you can debug
